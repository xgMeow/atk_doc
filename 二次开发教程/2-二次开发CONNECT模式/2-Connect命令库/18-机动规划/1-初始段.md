...
- 你可以使用 `SizeType`, `ShapeType` and `AnomType` 设置参数类型，如果你只是对参数进行了设置属性值，则会自动选择类型
例如你选择了 `Mean Motion` 类型，并对 `Semi-Major Axis` 设置值，那么类型将会变成 `Semi-Major Axis`

- 请保持数据正确性，若输入错误数据，数据类型将不可以转换
例如 输入 `sma` 为 `6 m`，须将数据修改正确后，例如修改为` 6800000 m`，才能转化为 `ApoapsisRadSize` 类型
