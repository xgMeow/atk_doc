...
::: tip 举例
```
SetPosition */Satellite/Satellite1 ECI "1 Jul 2021 09:00:00.000" 321 30 45 60
```
:::
...
::: info 说明
- `{Sequence}`有效值为123、132、213、231、312、321
...
作用：为对象添加一组 ECI J2000或者CBI的欧拉角姿态数据
...
对于火星：  GMM1,GMM2B,Mars50c,MRO110C <br><br>
...
<br>ThirdBodyGravityName包括：Mercury,Venus,Earth,Mars,Jupiter,Saturn,Uranus,Neptune,Pluto,Moon,Sun
:::
...
| `<Parameters>` | 说明 |
| -- | -- |
| `ReportOnFixedStep { On \| Off }` | 固定步长输出星历 |
| `OrbitEps <Value>` | 积分精度 |
:::
...
::: tip 举例
```
VO */Satellite/Satellite1 ScaleLog size 3
```
:::
...
```
```
WalkerDelta <SatObjectPath> Semimajoraxis <Value> Eccentricity <Value> Inclination <Value> RAAN <Value> ArgumentOfPerigee <Value> TureAnomaly <Value> {WalkerDeltaOptions}
```
:::
...
```
```
WalkerCustom <SatObjectPath> Semimajoraxis <Value> Eccentricity <Value> Inclination <Value> RAAN <Value> ArgumentOfPerigee <Value> TureAnomaly <Value> {WalkerCustomOptions}
```
:::
...
```
```
Rosette <SatObjectPath> Semimajoraxis <Value> Eccentricity <Value> Inclination <Value> RAAN <Value> ArgumentOfPerigee <Value> TureAnomaly <Value> {RosetteOptions}
```
...
```
```
Rosette / Semimajoraxis 6678137 Eccentricity 0 Inclination 28.5 RAAN 0 ArgumentOfPerigee 180 TureAnomaly 180 NumPlanes 20 TotalNumSats 20 Molecule 1 ColorByPlane Yes
```
:::
...
```
```
Flower <SatObjectPath> Semimajoraxis <Value> Eccentricity <Value> Inclination <Value> RAAN <Value> ArgumentOfPerigee <Value> TureAnomaly <Value> {FlowerOptions}
```
...
```
```
AsymmetricFlower <SatObjectPath> Semimajoraxis <Value> Eccentricity <Value> Inclination <Value> RAAN <Value> ArgumentOfPerigee <Value> TureAnomaly <Value> {AsymmetricFlowerOptions}
```
:::
...
```
```
AsymmetricFlower / Semimajoraxis 6678137 Eccentricity 0 Inclination 28.5 RAAN 0 ArgumentOfPerigee 180 TureAnomaly 180 TotalNumSats 16 ReturnCircle 16 ReturnDay 1 RAANIncrement 30 ColorByPlane Yes
```
:::