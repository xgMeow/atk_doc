...
![SWIG C++转 Java 接口封装与生成文件](media/3-SWIG将C++转成Java封装文件/image.png)

::: details SWIG接口代码（点击展开）
```swig
// example.i
//Java 模块名
%module example
...
%template(vector_IObjectLink)std::vector<IObjectLink*>;
//要忽略的文件/类/函数/变量
%ignore IAsGObjManage;
//包含需要调用的 C++声明文件，以本接口文件位置为基础
%include "../IAtkObj.h"
%include "Common.h"
%include "IAnimation.h"
...
```
swig -c++ -java -v -outdir D:\work\cppToJava\CppToJavaTest\javaJNI D:\work\cppToJava\CppToJavaTest\example.i
```

```
swig 命令格式（空格分开）：swig -源语言 -目标语言 -v(输出编译信息) –outdir java 文件输出目录 swig 编译文件目录
```
