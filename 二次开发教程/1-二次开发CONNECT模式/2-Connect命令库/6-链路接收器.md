...
| Model.Polarization.Refer- enceAxis | 设置参考轴，包括 `X Axis` ，`Y Axis` ，`Z Axis`                     |
| Model.Polarization.TiltAn- gle     | 设置极化角度，默认单位为 `rad`                                 |
| Model.LinkMarginType               | 设置链路裕量类型，包括 `None` ，`Flux Density` ，`RIP` ，`Rcvd Carrier Power` ，`C/N` ，`C/No`， `Eb/No` ，`BER` |
| Model.LinkMarginThresh- old        | 设置阈值，默认单位为 `dB`                                      |
...
| Property   | 说明                                                        |
| ---------- | ----------------------------------------------------------- |
| Properties | 获得当前属性定义的所有参数，包括他的最大 值，最小值，单位等 |
