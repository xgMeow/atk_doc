...
若选择属性Definition，Definition可输入{FOMType} <Parameters>
`{FOMType} <Parameters>`说明：

- `Simple On` 
 
- `CoverageTime Compute{ Total | MaxPerday | MinPerday | Perday | Percent }` 
 
- `AccessDuration Compute{ Sum | Maximum | Minimum | Average | PercentBelow | PercentAbove}`

- `RevisitTime Compute{ Total | Maximum | Minimum | Average }` 
 
- `TimeAverageGap` 
 
- `ResponseTime Compute{ Total | Maximum | Minimum | Average }`

- `NAsset Comput{ Maximum | Minimum | Average }`

- `NumberOfAccesses Compute{ Total | MaxPerday | MinPerday | AvgPerday }`
 
- `NumberOfGaps Compute{ Total | MaxPerday | MinPerday | AvgPerday }` 
 
- `AccessConstraint Compute{ Sum | Maximum | Minimum | Average }`

- `AccessConstraint Constraint{ Altitude | AngularRate | AzimuthAngle | ElevationAngle | Range | AzimuthRate | ElevationRate | RangeRate }`

- `LightConstraint Compute{ Sum | Maximum | Minimum | Average }`

- `LightConstraint Constraint{ SunGndElevationAngle | SunElevationAngle | SatelliteTargetSunAngle | TargetSatelliteSunAngle | LunarGndElevationAngle | LunarElevAngle | SatelliteTargetMoonAngle | TargetSatelliteMoonAngle }`
...
说明：`<ReportStyle>`包括Coverage为覆盖分析报告，`Figure Of Merit`为品质参数分析报告，`Satisfaction`为有效覆盖分析报告，`Daily Coverage`为每日覆盖分析报告，`Coverage Gaps`为未覆盖分析报告
...
::: note 举例
```
Cov_RM */CoverageDefinition/CoverageDefinition1 GridPoints
```
:::
...
- `CoverageTime Compute{ Total | MaxPerday | MinPerday | Perday | Percent }` 覆盖时间
 
- `AccessDuration Compute{ Sum | Maximum | Minimum | Average | PercentBelow | PercentAbove}` 访问时长

- `RevisitTime Compute{ Total | Maximum | Minimum | Average }` 重访时间
 
- `TimeAverageGap` 时间平均间隔
 
- `ResponseTime Compute{ Total | Maximum | Minimum | Average }` 响应时长
 
- `NAsset Compute{ Maximum | Minimum | Average }` 多重覆盖
 
- `NumberOfAccesses Compute{ Total | MaxPerday | MinPerday | AvgPerday }` 访问次数

- `NumberOfGaps Compute{ Total | MaxPerday | MinPerday | AvgPerday }` 覆盖间隔次数
 
- `AccessConstraint Compute{ Sum | Maximum | Minimum | Average }` 可见性约束

- `AccessConstraint Constraint{ Altitude | AngularRate | AzimuthAngle | ElevationAngle | Range | AzimuthRate | ElevationRate | RangeRate }` 可见性约束

- `LightConstraint Compute{ Sum | Maximum | Minimum | Average }` 光照约束
 
- `LightConstraint Constraint{ SunGndElevationAngle | SunElevationAngle | AatelliteTargetSunAngle | TargetSatelliteSunAngle | LunarGndElevationAngle | LunarElevAngle | SatelliteTargetMoonAngle | TargetSatelliteMoonAngle }` 光照约束
